...
def query(index, query_str):
    ...
    r = requests.post("%s/%s/_search" % (es_url, index), data=query_str, verify=False)
    result = r.json()
    ...
    for hit in result["hits"]["hits"]:
        ...
        if "track_number" not in hit.get("fields", {}).get("metadata", {}):
            hit["fields"]["metadata"]["track_number"] = hit["fields"]["metadata"].get(
                "trackNumber", ""
            )
        ...
        if len(hit["fields"]["urls"]) == 0:
            ...
            hit["fields"]["urls"] = [
                url_for(
                    "services/dataset.resolve_url", index=hit["_index"], id=hit["_id"]
                )
            ]
