...
def urs_user_verified(code):
    ...
    auth_val = base64.b64encode(
        "{}:{}".format(app.config["URS_APP_ID"], app.config["URS_APP_PASSWORD"])
    )
    ...
    # exchange
    r = requests.post(
        urs_url, data=payload, headers={"Authorization": "Basic {}".format(auth_val)}
    )
    try:
        r.raise_for_status()
    ...
    r = requests.get(
        ...
        headers={
            "Authorization": "{} {}".format(
                packet["token_type"], packet["access_token"]
            )
        },
    )
    r.raise_for_status()
