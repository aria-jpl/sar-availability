...
def edit_user_rule():
    ...
    if r.status_code != 200:
        app.logger.debug(
            "Failed to update rule for %s. Got status code %d:\n%s"
            % (id, r.status_code, json.dumps(result, indent=2))
        )
    r.raise_for_status()
...
def process_this():
    ...
    if url_match:
        ...
        return jsonify({"success": True, "message": "", "html": html})
    ...
    r = requests.post(
        "%s/%s/%s/"
        % (app.config["ES_URL"], app.config["USER_RUNS_HISTORY_INDEX"], "user_run"),
        data=json.dumps(user_run),
    )
    result = r.json()
    if r.status_code != 201:
        app.logger.debug(
            "Failed to record user run for %s. Got status code %d:\n%s"
            % (g.user.id, r.status_code, json.dumps(result, indent=2))
        )
    r.raise_for_status()
...
def monitor_job():
    ...
    if orch_task_id is None:
        return jsonify({"success": False, "message": "Task ID not specified."}), 500
...
def monitor_jobs():
    ...
    if tag is None:
        return jsonify({"success": False, "message": "Tag not specified."}), 500
    username = request.args.get("username", None)
    if username is None:
        return jsonify({"success": False, "message": "User not specified."}), 500
