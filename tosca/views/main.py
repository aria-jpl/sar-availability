...
def internal_server_error(e):
    return (
        render_template(
            ...
            title="TOSCA: Encountered Error",
            current_year=datetime.now().year,
            error_msg="Error code 500: " + str(e),
        ),
        500,
    )
...
def unimplemented(e):
    return (
        render_template(
            ...
            title="TOSCA: Encountered Error",
            current_year=datetime.now().year,
            error_msg="Error code 501: " + str(e),
        ),
        501,
    )
...
def sitemap():
    ...
    dt = datetime.utcnow()
    pages = [
        [url_for("views/main.index", _external=True), dt],
...
def logout():
    ...
    if app.config.get("URS_ENABLED", False):
        ...
        return redirect(
            auth_url.format(app.config["URS_URL"], app.config["URS_REDIRECT_URL"])
        )
